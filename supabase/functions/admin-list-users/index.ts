...
Deno.serve(async (req) => {
  ...
  try {
    ...
    if (!h?.startsWith('Bearer ')) return new Response(JSON.stringify({ error: 'Authentication required' }), { status: 401, headers: { ...cors, 'Content-Type': 'application/json' } });
    ...
    const { data: me } = await admin.from('staff').select('staff_role:staff_role(code)').eq('auth_user_id', user.id).maybeSingle();
    if (!['SUPER_ADMIN', 'ADMIN'].includes(me?.staff_role?.code)) return new Response(JSON.stringify({ error: 'Administrator access required' }), { status: 403, headers: { ...cors, 'Content-Type': 'application/json' } });
