-- 035_change_set_apply_engine.sql
-- Final table-specific, allow-listed, transactional promotion implementation.
-- Uses jsonb_populate_record only after field mappings are validated against real columns.
...
begin
  ...
  for item in select * from intake.change_set_item where change_set_id=p_change_set_id and status='APPROVED' order by id for update loop
    ...
    if table_name not in ('client','programme','quotation','purchase_order','invoice','payment') then
      raise exception 'Target table is not allow-listed: %', table_name;
    end if;
    if item.proposed_payload is null or jsonb_typeof(item.proposed_payload) <> 'object' then
      raise exception 'Invalid payload for change-set item %', item.id;
    end if;

    -- Build the mutation strictly from server-owned field mappings and real table columns.
    select string_agg(format('%I',m.target_field), ', ' order by m.target_field),
           string_agg(format('t.%I = src.%I',m.target_field,m.target_field), ', ' order by m.target_field)
      into cols, sets
    from intake.field_mapping m
    join pg_attribute a on a.attrelid=format('public.%s',table_name)::regclass and a.attname=m.target_field and a.attnum>0 and not a.attisdropped
    where m.target_table=table_name
      and m.target_field <> all(protected)
      and item.proposed_payload ? m.source_field;

    if cols is null or sets is null then
      raise exception 'No approved mapped fields for change-set item %', item.id;
    end if;

    if item.operation='UPDATE' then
      if item.target_id is null then raise exception 'UPDATE item % has no target_id', item.id; end if;
      -- Optimistic concurrency: expected hash must be supplied by comparison layer.
      -- The hash contract is checked against the canonical row using deterministic text serialization.
      if item.expected_existing_hash is null then raise exception 'UPDATE item % missing expected_existing_hash', item.id; end if;
      ...
      execute sql using item.proposed_payload,item.target_id into target_id;
      get diagnostics affected = row_count;
      if affected <> 1 then raise exception 'UPDATE target % not found or could not be updated',item.target_id; end if;
      ...
      execute sql using item.proposed_payload into target_id;
      get diagnostics affected = row_count;
    ...
    values(cs.batch_id,item.record_id,table_name,target_id,item.operation,item.proposed_payload,coalesce(item.proposed_payload->'changed_fields','[]'::jsonb),'Approved import change-set promotion',1.0,cs.approved_by);
...
comment on function intake.apply_change_set(bigint) is 'Privileged transactional promotion. Server-owned field mappings and canonical table types constrain every NEW/UPDATE mutation.';
