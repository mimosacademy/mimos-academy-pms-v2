-- 011_rls.sql
-- Browser access is governed by Supabase Auth + RLS. No service key belongs in Vite.
...
BEGIN
  FOREACH t IN ARRAY ARRAY[
    ...
    'staff','account','client','client_contact','programme','quotation','purchase_order','invoice','payment','invoice_payment_allocation','opportunity','action_item','training_stat','participant','source_file','import_batch','audit_log','data_conflict','completeness_score','staff_alias','client_alias',
    'stg_invoice','stg_quotation','stg_funnel','stg_action_item','stg_training_stat','stg_cost_of_sales'
  ] LOOP
...
-- Tighten especially sensitive audit/conflict tables: viewers cannot inspect internal
-- audit evidence or unresolved data-quality queues.
...
-- Staff profiles are readable by authenticated users but only admins can mutate them.
...
-- Grant table access required by PostgREST; RLS remains the authorization boundary.
