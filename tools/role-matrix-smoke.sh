...
ADMIN_TOKEN="$(login _superusers "$ADMIN_EMAIL" "$ADMIN_PASSWORD")"
roles=(manager finance sales programme_pic trainer viewer)
for role in "${roles[@]}"; do
  email="ci-${role}@example.invalid"; pw='CiRole-Password-2026-Strong!'
  curl -fsS -X POST -H "Authorization: $ADMIN_TOKEN" -H 'Content-Type: application/json' -d "{\"email\":\"$email\",\"password\":\"$pw\",\"passwordConfirm\":\"$pw\",\"name\":\"CI ${role}\",\"role\":\"$role\",\"verified\":true}" "$BASE_URL/api/collections/users/records" > "$TMP_DIR/$role.json"
done
...
login_role(){ login users "ci-$1@example.invalid" 'CiRole-Password-2026-Strong!'; }
for role in "${roles[@]}"; do
...
# List rules are filters and can legitimately return HTTP 200 with zero records.
...
# Valid client payload: sales may create; viewer must be denied. PocketBase create-rule denial is HTTP 400.
...
# Build a real programme fixture for training and finance tests.
