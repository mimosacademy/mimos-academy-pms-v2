...
login() {
  ...
  if [[ "$code" != "200" ]]; then
    echo "Authentication failed: $collection/$identity HTTP=$code" >&2
    printf '%s\n' "$payload" >&2
    return 1
  fi
...
ADMIN_FILTER="$(python3 - "$ADMIN_EMAIL" <<'PY'
...
print(json.dumps({"email":email,"password":password,"passwordConfirm":password,"name":f"CI {role}","role":role,"team":"MASB_Team"}))
PY
)"
  response="$(api -X POST -H "Authorization: $ADMIN_TOKEN" -H 'Content-Type: application/json' -d "$payload" "$BASE_URL/api/collections/users/records")"
  code="$(status "$response")"
  if [[ "$code" != 2* ]]; then echo "Failed to provision $role HTTP=$code"; body "$response"; exit 1; fi
  body "$response" > "$TMP_DIR/$role.json"
done

role_id() { json_id < "$TMP_DIR/$1.json"; }
login_role() { login users "ci-$1@example.com" "$ROLE_PASSWORD"; }

for role in "${roles[@]}"; do
  token="$(login_role "$role")"
  id="$(role_id "$role")"
  response="$(api -H "Authorization: $token" "$BASE_URL/api/collections/users/records/$id")"
  [[ "$(status "$response")" == "200" ]] || { echo "Role view failed: $role"; body "$response"; exit 1; }
...
[[ "$viewer_total" == "0" ]] || { echo 'Viewer unexpectedly received user records'; body "$viewer_response"; exit 1; }

client_payload="$(python3 - "$ADMIN_ID" <<'PY'
import json,sys
print(json.dumps({"name":"CI Sales Client","status":"Active","createdBy":sys.argv[1]}))
PY
)"
expect_status 200 -X POST -H "Authorization: $sales" -H 'Content-Type: application/json' -d "$client_payload" "$BASE_URL/api/collections/clients/records"
expect_status 400 -X POST -H "Authorization: $viewer" -H 'Content-Type: application/json' -d "$(python3 - "$ADMIN_ID" <<'PY'
import json,sys
...
CID="$(printf '%s' "$client_json" | json_id)"

programme_json="$(create_record "$manager" programmes "$(python3 - "$CID" "$ADMIN_ID" <<'PY'
import json,sys
print(json.dumps({"client":sys.argv[1],"createdBy":sys.argv[2],"code":"CI-FIN-001","title":"CI Financial Programme","status":"Scheduled","contractValue":1000}))
PY
)")"
PID="$(printf '%s' "$programme_json" | json_id)"

expect_status 200 -X POST -H "Authorization: $trainer" -H 'Content-Type: application/json' -d "$(python3 - "$PID" "$ADMIN_ID" <<'PY'
import json,sys
...
expect_status 400 -X POST -H "Authorization: $viewer" -H 'Content-Type: application/json' -d "$(python3 - "$PID" "$ADMIN_ID" <<'PY'
import json,sys
...
print(json.dumps({"programme":programme,"client":client,"createdBy":created_by,"invoiceNo":"CI-INV-001","description":"CI integrity","amount":1000,"paidAmount":0,"status":"Unpaid"}))
PY
)")"
IID="$(printf '%s' "$invoice_json" | json_id)"

create_record "$finance" payments "$(python3 - "$IID" "$PID" "$CID" "$ADMIN_ID" <<'PY'
import json,sys
invoice,programme,client,created_by=sys.argv[1:]
print(json.dumps({"invoice":invoice,"programme":programme,"client":client,"createdBy":created_by,"paymentNo":"CI-PAY-001","amount":600,"method":"Bank Transfer","status":"Completed"}))
PY
)" >/dev/null

expect_status 400 -X POST -H "Authorization: $finance" -H 'Content-Type: application/json' -d "$(python3 - "$IID" "$PID" "$CID" "$ADMIN_ID" <<'PY'
import json,sys
invoice,programme,client,created_by=sys.argv[1:]
print(json.dumps({"invoice":invoice,"programme":programme,"client":client,"createdBy":created_by,"paymentNo":"CI-PAY-002","amount":401,"method":"Bank Transfer","status":"Completed"}))
PY
)" "$BASE_URL/api/collections/payments/records"
expect_status 400 -X PATCH -H "Authorization: $finance" -H 'Content-Type: application/json' -d '{"amount":500}' "$BASE_URL/api/collections/invoices/records/$IID"

echo 'Role provisioning, 7-role authorization and financial integrity E2E smoke tests passed.'
