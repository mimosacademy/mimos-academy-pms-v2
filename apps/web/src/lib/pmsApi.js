...
const TABLES = Object.freeze({
  ...
  trainingDeliveries: 'training_delivery', trainingStatistics: 'training_statistics', participants: 'participant',
...
const throwDatabaseError = (operation, table, error) => {
  ...
  if (message.includes('permission') || message.includes('row-level security') || error.code === '42501') throw new Error('You are not authorized to perform this operation.');
  if (error.code === '23505') throw new Error('Duplicate transaction detected. The operation may already have been recorded.');
...
/**
 * Payment-specific transaction boundary. operation_id is generated by the UI
 * once per form instance and is reused on retry. A duplicate operation_id is
 * treated as an idempotent replay and returns the existing payment row.
 */
export async function createPaymentRecord(payload = {}) {
  ...
  if (error.code === '23505') {
    const { data: existing, error: lookupError } = await supabase
      ...
      .maybeSingle();
    throwDatabaseError('Payment idempotency lookup', 'payment', lookupError);
