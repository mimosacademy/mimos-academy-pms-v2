...
const getAuthenticatedUser = async () => {
  ...
  if (error || !data?.user?.id) throw new Error('Authentication required. Please sign in again.');
  return data.user;
};

// Database details/hints/codes are intentionally not returned to end users.
const throwDatabaseError = (operation, table, error) => {
  ...
  throw new Error(`${operation} failed for ${table}. Please try again or contact an administrator.`);
};
...
export async function listRecords(table, { select = '*', filters = {}, orderBy = 'created_at', ascending = false, page = 1, pageSize = 50 } = {}) {
  ...
  const safePage = sanitizePage(page), safePageSize = sanitizePageSize(pageSize);
  const from = (safePage - 1) * safePageSize, to = from + safePageSize - 1;
  let query = supabase.from(table).select(select, { count: 'exact' }).order(orderBy, { ascending }).range(from, to);
  for (const [column, value] of Object.entries(filters)) if (value !== undefined && value !== null && value !== '') query = query.eq(column, value);
...
export async function updateRecord(table, id, payload = {}) {
  ...
  delete row.created_by; delete row.actor_id; delete row.actor_name;
...
export function subscribeToTable(table, callback, { event = '*', filter } = {}) {
  assertSafeTable(table);
  const changes = supabase.channel(`pms:${table}:${crypto.randomUUID()}`).on('postgres_changes', { event, schema: 'public', table, ...(filter ? { filter } : {}) }, callback);
  changes.subscribe((status) => { if (status === 'CHANNEL_ERROR' || status === 'TIMED_OUT') console.warn(`Realtime subscription unavailable for ${table}.`); });
...
export async function uploadDocument(file, { programmeId } = {}) {
  ...
  if (!Number.isInteger(Number(programmeId)) || Number(programmeId) < 1) throw new Error('A valid programme is required before uploading a document.');
  const safeName = file.name.replace(/[^a-zA-Z0-9._-]/g, '_').slice(0, 180) || 'document';
  const path = `programmes/${Number(programmeId)}/${crypto.randomUUID()}-${safeName}`;
  const { error } = await supabase.storage.from('pms-documents').upload(path, file, { cacheControl: '3600', upsert: false, contentType: file.type || 'application/octet-stream' });
  if (error) throw new Error('Document upload failed.');
  return path;
}

export async function createSignedDocumentUrl(path, expiresIn = 3600) {
  if (!path || !/^programmes\/\d+\/[^/]+$/.test(path)) throw new Error('Invalid document path.');
  const safeExpiry = Math.min(Math.max(Number(expiresIn) || 3600, 60), 3600);
  const { data, error } = await supabase.storage.from('pms-documents').createSignedUrl(path, safeExpiry);
  if (error) throw new Error('Signed document URL could not be created.');
