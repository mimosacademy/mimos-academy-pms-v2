...
import { Banknote, Download, HandCoins, Percent, Target, FileSpreadsheet } from 'lucide-react';
import { toast } from 'sonner';

const STATUS_COLORS = { Paid: '#16a34a', Partial: '#d97706', Unpaid: '#94a3b8', Overdue: '#dc2626' };

const REPORTS = {
  R1: { view: 'v_r1_income_statement', filename: 'R1-income-statement' },
  R2: { view: 'v_r2_training_stats', filename: 'R2-overall-training' },
  R3: { view: 'v_r3_funnel_pipeline', filename: 'R3-funnel-pipeline' },
};
...
export default function ReportsPage() {
  ...
  const revenueCollectionChartData = monthlyFinancials.map((month) => ({ month: month.month, revenue: decimalToNumberForDisplay(month.revenue || '0'), collection: decimalToNumberForDisplay(month.collection || '0') }));
  ...
  const exportCsv = () => {
    ...
    const url = URL.createObjectURL(new Blob([csv], { type: 'text/csv;charset=utf-8' }));
    const anchor = document.createElement('a'); anchor.href = url; anchor.download = `pms-report-${new Date().toISOString().slice(0, 10)}.csv`; document.body.appendChild(anchor); anchor.click(); anchor.remove(); URL.revokeObjectURL(url);
  };

  const exportReport = async (code) => {
    const config = REPORTS[code];
    ...
    try {
      ...
      toast.error(`Unable to export ${code}`);
    } finally {
      setExporting('');
    }
  };
  ...
  return <div>
    ...
    <PageHeader title="Programme Reports" description="Programme revenue, collection and order-book analytics.">
      ...
      <Button className="bg-violet-600 hover:bg-violet-700" onClick={exportCsv}><Download className="mr-2 h-4 w-4" /> Export CSV</Button>
    </PageHeader>

    <section className="mb-6 rounded-xl border bg-white p-5 shadow-sm">
      <div className="mb-3 flex items-center gap-2"><FileSpreadsheet className="h-5 w-5 text-violet-600" /><div><h2 className="text-sm font-semibold text-slate-900">Official Report Exports</h2><p className="text-xs text-slate-500">Generated directly from the secured Supabase reporting views.</p></div></div>
      <div className="flex flex-wrap gap-3">
        {Object.keys(REPORTS).map((code) => <Button key={code} variant="outline" disabled={Boolean(exporting)} onClick={() => exportReport(code)}>{exporting === code ? 'Exporting…' : `Export ${code}`}</Button>)}
      </div>
    </section>
