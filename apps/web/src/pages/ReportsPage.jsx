...
export default function ReportsPage() {
  ...
  const revenueCollectionChartData = monthlyFinancials.map((month) => ({
    month: month.month,
    revenue: decimalToNumberForDisplay(month.revenue || '0'),
    collection: decimalToNumberForDisplay(month.collection || '0'),
  }));
  const exportCsv = () => {
  ...
  return <div>
    ...
    <div className="grid grid-cols-1 gap-6 xl:grid-cols-3">
      <section className="rounded-xl border bg-white shadow-sm xl:col-span-2">
        <header className="border-b px-5 py-4"><h2 className="text-sm font-semibold text-slate-900">Revenue vs Collection</h2><p className="mt-0.5 text-xs text-slate-400">Monthly billed revenue against payments collected</p></header>
        <div className="h-80 p-5"><ResponsiveContainer width="100%" height="100%"><BarChart data={revenueCollectionChartData}><CartesianGrid strokeDasharray="3 3" vertical={false} /><XAxis dataKey="month" /><YAxis tickFormatter={(value) => `${Math.round(Number(value || 0) / 1000)}K`} /><Tooltip content={chartTooltip} /><Legend /><Bar dataKey="revenue" name="Revenue" fill="#7c3aed" /><Bar dataKey="collection" name="Collection" fill="#16a34a" /></BarChart></ResponsiveContainer></div>
      </section>
      <section className="rounded-xl border bg-white shadow-sm">
        <header className="border-b px-5 py-4"><h2 className="text-sm font-semibold text-slate-900">Receivables by Status</h2><p className="mt-0.5 text-xs text-slate-400">Invoice count by payment status</p></header>
        <div className="h-80 p-5"><ResponsiveContainer width="100%" height="100%"><PieChart><Pie data={invoiceStatusData} dataKey="count" nameKey="name" innerRadius="55%" outerRadius="80%" paddingAngle={3} strokeWidth={0}>{invoiceStatusData.map((status) => <Cell key={status.name} fill={STATUS_COLORS[status.name]} />)}</Pie><Tooltip /><Legend /></PieChart></ResponsiveContainer></div>
      </section>
    </div>
