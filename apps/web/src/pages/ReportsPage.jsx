...
const chartTooltip = ({ active, payload, label }) => {
  ...
  return <div className="rounded-lg border bg-white px-3 py-2 text-xs shadow-md">{label && <p className="mb-1 font-semibold text-slate-800">{label}</p>}{payload.map((p) => <p key={p.dataKey ?? p.name} className="text-slate-600">{p.name}: <span className="font-semibold text-slate-900">{formatRM(Number(p.value || 0))}</span></p>)}</div>;
};

export default function ReportsPage() {
  const { invoices = [], monthlyFinancials = [], pipelineValue = 0 } = usePmsData();
  const ytdRevenue = monthlyFinancials.reduce((sum, month) => sum + Number(month.revenue || 0), 0);
  const ytdCollection = monthlyFinancials.reduce((sum, month) => sum + Number(month.collection || 0), 0);
  ...
  const topClients = Object.entries(invoices.reduce((acc, invoice) => { const key = invoice.clientName || 'Unknown'; acc[key] = (acc[key] ?? 0) + Number(invoice.totalAmount || invoice.amount || 0); return acc; }, {})).map(([name, value]) => ({ name, value })).sort((a, b) => b.value - a.value).slice(0, 6);
  const maxClientValue = topClients[0]?.value || 1;

  const exportCsv = () => {
    const rows = [['Month', 'Revenue', 'Collection', 'Variance'], ...monthlyFinancials.map((month) => [month.month, Number(month.revenue || 0), Number(month.collection || 0), Number(month.collection || 0) - Number(month.revenue || 0)])];
