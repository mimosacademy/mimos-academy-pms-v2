...
export default function ReportsPage() {
  ...
  const invoiceStatusData = ['Paid', 'Partial', 'Unpaid', 'Overdue'].map((status) => ({ name: status, count: invoices.filter((i) => i.status === status).length }));
  const topClients = Object.entries(invoices.reduce((acc, i) => { const key = i.clientName || 'Unknown'; acc[key] = (acc[key] ?? 0) + Number(i.totalAmount || i.amount || 0); return acc; }, {})).map(([name, value]) => ({ name, value })).sort((a, b) => b.value - a.value).slice(0, 6));
  const maxClientValue = topClients[0]?.value || 1;

  const exportCsv = () => {
    const rows = [['Month', 'Revenue', 'Collection', 'Variance'], ...monthlyFinancials.map((m) => [m.month, m.revenue, m.collection, m.collection - m.revenue])];
    const csv = rows.map((r) => r.map((v) => `"${String(v).replaceAll('"', '""')}"`).join(',')).join('\n');
    const url = URL.createObjectURL(new Blob([csv], { type: 'text/csv;charset=utf-8' }));
    const a = document.createElement('a'); a.href = url; a.download = `pms-report-${new Date().toISOString().slice(0, 10)}.csv`; a.click(); URL.revokeObjectURL(url);
  ...
  return <div>
    ...
    <PageHeader title="Programme Reports" description="Programme revenue, collection and order-book analytics."><Button variant="outline" onClick={exportPrint}><Download className="mr-2 h-4 w-4" /> Print / PDF</Button><Button className="bg-violet-600 hover:bg-violet-700" onClick={exportCsv}><Download className="mr-2 h-4 w-4" /> Export CSV</Button></PageHeader>
