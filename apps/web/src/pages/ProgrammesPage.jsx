...
import { ExternalLink, CalendarRange, CheckCircle2, ClipboardList, GraduationCap, Plus } from 'lucide-react';
import { usePmsData } from '@/contexts/PmsDataContext';
import { formatDate, formatRM, formatRMCompact } from '@/lib/format';

const demoAction = () => toast.info('Schedule export is available after data is loaded.');
...
export default function ProgrammesPage() {
  ...
  const columns = [
    ...
    { key: 'startDate', label: 'Schedule', className: 'whitespace-nowrap text-slate-500', render: (p) => `${formatDate(p.startDate)} – ${formatDate(p.endDate)}` },
  ...
  return <div>
    <Helmet><title>Programmes — MIMOS Academy PMS</title><meta name="description" content="Central programme hub for MIMOS Academy." /></Helmet>
    <PageHeader title="Programmes" description="The central business entity — every opportunity, PO, delivery and collection links here.">
      <Button variant="outline" onClick={demoAction}><CalendarRange className="mr-2 h-4 w-4" /> Export Schedule</Button>
      <EntityDialog collection="programmes" title="New Programme" description="Register a new programme and assign its initial delivery status." triggerLabel="New Programme" fields={[
        { name: 'client', label: 'Client', type: 'relation', relation: 'clients', required: true }, { name: 'code', label: 'Programme Code', required: true }, { name: 'title', label: 'Programme Title', required: true, full: true }, { name: 'category', label: 'Category' },
        { name: 'programmeCategory', label: 'Programme Type', type: 'select', options: [{ value: 'In-House', label: 'In-House' }, { value: 'Public', label: 'Public' }, { value: 'Workshop', label: 'Workshop' }] },
        ...
        { name: 'participants', label: 'Participants', type: 'number', min: 0 }, { name: 'progress', label: 'Progress (%)', type: 'number', min: 0, max: 100 }, { name: 'contractValue', label: 'Contract Value (RM)', type: 'number', min: 0, step: '0.01' }, { name: 'accountManager', label: 'Account Manager' }, { name: 'sessionsPlanned', label: 'Sessions Planned', type: 'number', min: 0 }, { name: 'sessionsDelivered', label: 'Sessions Delivered', type: 'number', min: 0 },
      ]} />
    </PageHeader>
    ...
    <DataTable columns={columns} data={programmes} searchKeys={['title', 'code', 'clientName', 'trainer', 'pic']} searchPlaceholder="Search programmes…" filters={[{ key: 'status', label: 'Status', options: ['Scheduled', 'In Progress', 'Completed', 'On Hold'] }, { key: 'category', label: 'Category', options: ['AI & Data', 'Cybersecurity', 'Cloud', 'Leadership', 'Emerging Tech'] }]} emptyTitle="No programmes found" emptyDescription="No programmes match your current search or filters." />
  </div>;
}
