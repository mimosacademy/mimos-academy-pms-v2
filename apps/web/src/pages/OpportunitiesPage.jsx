...
const exportOpportunities = (opportunities) => {
  const headers = ['Opportunity', 'Client', 'Sector', 'Forecast', 'Weighted', 'Status', 'Probability', 'Expected Close', 'Account Manager'];
  const escape = (v) => `"${String(v ?? '').replaceAll('"', '""')}"`;
  const csv = [headers, ...opportunities.map((o) => [o.title, o.clientName, o.sector, o.value, o.weighted, o.stage, o.probability, o.expectedClose, o.accountManager])]
    .map((row) => row.map(escape).join(',')).join('\n');
  const url = URL.createObjectURL(new Blob([csv], { type: 'text/csv;charset=utf-8;' }));
  const a = document.createElement('a'); a.href = url; a.download = `mimos-academy-opportunities-${new Date().toISOString().slice(0, 10)}.csv`; a.click(); URL.revokeObjectURL(url);
};
...
export default function OpportunitiesPage() {
  ...
  const columns = [
    ...
    { key: 'probability', label: 'Probability', render: (o) => <div className="flex w-24 items-center gap-2"><div className="h-1.5 w-full overflow-hidden rounded-full bg-slate-100"><div className="h-full rounded-full bg-violet-600" style={{ width: `${Math.min(100, Math.max(0, o.probability))}%` }} /></div><span className="text-xs font-medium text-slate-500">{o.probability}%</span></div> },
  ...
  return <div>
    ...
    <PageHeader title="Opportunities" description="First step in the programme flow — qualify demand before quotation.">
      <Button variant="outline" onClick={() => exportOpportunities(opportunities)}><Download className="mr-2 h-4 w-4" /> Export</Button>
      <EntityDialog collection="opportunities" title="New Opportunity" description="Register a new sales opportunity in the pipeline." triggerLabel="New Opportunity" fields={[
        { name: 'client', label: 'Client', type: 'relation', relation: 'clients', required: true }, { name: 'title', label: 'Opportunity / Project', required: true, full: true },
        { name: 'forecastValue', label: 'Forecast Value (RM)', type: 'number', min: 0, step: '0.01', required: true }, { name: 'opportunityStatus', label: 'Status', type: 'select', options: [
          { value: 'Early engagement', label: 'Early engagement' }, { value: 'Qualified lead/Tender in progress', label: 'Qualified lead/Tender in progress' }, { value: 'Proposal/Tender submitted', label: 'Proposal/Tender submitted' }, { value: 'Negotiation stage', label: 'Negotiation stage' }, { value: 'Verbal commitment', label: 'Verbal commitment' }, { value: 'Contract signed/PO issued', label: 'Contract signed/PO issued' }, { value: 'Lost/No-go', label: 'Lost/No-go' },
        ] },
        { name: 'probability', label: 'Probability (%)', type: 'number', min: 0, max: 100, required: true }, { name: 'expectedClose', label: 'Expected Close', type: 'date' },
        { name: 'sector', label: 'Sector', type: 'select', options: [{ value: 'Government', label: 'Government' }, { value: 'Private', label: 'Private' }] },
        { name: 'accountManager', label: 'Account Manager' }, { name: 'salesman', label: 'Salesperson' }, { name: 'year', label: 'Year', type: 'number', min: 2020, max: 2100 },
      ]} />
    </PageHeader>
    ...
    <DataTable columns={columns} data={opportunities} searchKeys={['title', 'clientName', 'accountManager', 'sector']} searchPlaceholder="Search opportunities…" filters={[{ key: 'stage', label: 'Status', options: ['Early engagement', 'Qualified lead/Tender in progress', 'Proposal/Tender submitted', 'Negotiation stage', 'Verbal commitment', 'Contract signed/PO issued', 'Lost/No-go'] }, { key: 'sector', label: 'Sector', options: ['Government', 'Private'] }, { key: 'accountManager', label: 'Account Manager', options: [...new Set(opportunities.map((o) => o.accountManager).filter(Boolean))] }]} emptyTitle="No opportunities found" emptyDescription="No opportunities match your current search or filters." />
  </div>;
}
