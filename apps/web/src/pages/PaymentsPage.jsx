...
const PAYMENT_WRITE_ROLES = new Set(['super_admin', 'manager', 'finance']);

export default function PaymentsPage() {
  const { payments } = usePmsData();
  const { user } = useAuth();
  const canRecordPayment = PAYMENT_WRITE_ROLES.has(user?.role);
  const now = new Date();
  ...
  const thisMonth = completed.filter((p) => p.date?.startsWith(currentMonth)).reduce((s, p) => decimalAdd(s, p.amount), '0');
  ...
  return (
    <div>
      <Helmet><title>Payments — MIMOS Academy PMS</title><meta name="description" content="Payment collection records for MIMOS Academy — receipts, methods, references and reconciliation status." /></Helmet>
      <PageHeader title="Payment Collection" description="Final step in the programme flow — collections against programme invoices.">
        {canRecordPayment && <EntityDialog collection="payments" title="Record Payment" description="Record a payment received and link it to the invoice." triggerLabel="Record Payment" fields={[
          { name: 'invoice', label: 'Invoice', type: 'relation', relation: 'invoices', required: true },
          ...
          { name: 'method', label: 'Method', type: 'select', options: [{ value: 'Bank Transfer', label: 'Bank Transfer' }, { value: 'Cheque', label: 'Cheque' }, { value: 'Online Banking', label: 'Online Banking' }, { value: 'Credit Card', label: 'Credit Card' }] },
          { name: 'date', label: 'Payment Date', type: 'date' },
          { name: 'reference', label: 'Reference' },
          { name: 'status', label: 'Status', type: 'select', options: [{ value: 'Completed', label: 'Completed' }, { value: 'Pending', label: 'Pending' }, { value: 'Failed', label: 'Failed' }] },
        ]} />}
      </PageHeader>
      ...
      <DataTable columns={columns} data={payments} searchKeys={['paymentNo', 'invoiceNo', 'clientName', 'reference']} searchPlaceholder="Search payments…" filters={[{ key: 'status', label: 'Status', options: ['Completed', 'Pending'] }, { key: 'method', label: 'Method', options: ['Bank Transfer', 'Cheque', 'Online Banking', 'Credit Card'] }]} emptyTitle="No payments found" emptyDescription="No payments match your current search or filters." />
    </div>
  );
}
